...
float MyModel::generate_scale(cv::Mat& image, const std::vector<int>& target_size) {
...
float MyModel::letterbox(cv::Mat& input_image, cv::Mat& output_image, const std::vector<int>& target_size) {
...
torch::Tensor MyModel::xyxy2xywh(const torch::Tensor& x) {
...
torch::Tensor MyModel::xywh2xyxy(const torch::Tensor& x) {
	auto y = torch::empty_like(x);
...
torch::Tensor MyModel::nms(const torch::Tensor& bboxes, const torch::Tensor& scores, float iou_threshold) {
	if (bboxes.numel() == 0)
...
torch::Tensor MyModel::non_max_suppression(torch::Tensor& prediction, float conf_thres, float iou_thres, int max_det) {
...
torch::Tensor MyModel::clip_boxes(torch::Tensor& boxes, const std::vector<int>& shape) {
...
torch::Tensor MyModel::scale_boxes(const std::vector<int>& img1_shape, torch::Tensor& boxes, const std::vector<int>& img0_shape) {
...
// model_path = "D:/new-coding/testdata/best.torchscript"
std::vector<Shape> MyModel::runModel(const std::string& model_path, cv::Mat image) {
	// 存储所有的标注为shape类
	...
	std::vector<std::string> classes{ "CEC", "RBC", "SEC", "EC","NEC", "LC", "MC" };
	...
	try {
		...
		/*cv::Mat image = cv::imread("D:/new-coding/testdata/new-images/000720_960_1440.jpg");*/
		cv::Mat input_image;
		...
		// NMS
		auto keep = non_max_suppression(output, 0.25, 0.7)[0];
		...
		for (int i = 0; i < keep.size(0); i++) {
			double x1 = static_cast<double>(keep[i][0].item().toInt());
			double y1 = static_cast<double>(keep[i][1].item().toInt());
			double x2 = static_cast<double>(keep[i][2].item().toInt());
			double y2 = static_cast<double>(keep[i][3].item().toInt());
			int cls = keep[i][5].item().toInt();

			Shape shape(classes[cls], 0); // shape_type = 0 (rectangle)
			shape.setPoints({ Point(x1, y1), Point(x2, y2) });
