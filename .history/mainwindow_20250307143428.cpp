    ...
    , ui(new Ui::MainWindow)
{
    ...
    // 初始化保存路径栏
    saveLocationBar = new QWidget(this); // 初始�? saveLocationBar
    ...
    QPushButton *selectFolderButton = new QPushButton("选择文件�?", saveLocationBar);
    // connect(selectFolderButton, &QPushButton::clicked, this, &MainWindow::selectSceneImageSaveFolder);

    QPushButton *saveImageButton = new QPushButton("保存为图�?", saveLocationBar);
    ...
    // 初始化工具栏
    QToolBar *toolbar = addToolBar("我的工具�?");
    ...
    QAction *openFolderAction = new QAction(QIcon(":/IMG/icons/folder-open-image.png"), "打开文件�?", this);
    // connect(openFolderAction, &QAction::triggered, this, &MainWindow::openFolder);
    toolbar->addAction(openFolderAction);

    // Action：上一�?
    QAction *action_previous = new QAction(QIcon(":/IMG/icons/arrow-180.png"), "上一�?", this);
    action_previous->setStatusTip("上一�?");
    ...
    // Action：下一�?
    QAction *action_next = new QAction(QIcon(":/IMG/icons/arrow.png"), "下一�?", this);
    action_next->setStatusTip("下一�?");
    ...
    // Action: 添加按钮：控制生成三角形
    QAction *triangle_action = new QAction(QIcon(":/IMG/icons/control-090.png"), "生成三角�?", this);
    triangle_action->setCheckable(true);  // 设置为可切换状�?
    // connect(triangle_action, &QAction::triggered, this, &MainWindow::toggle_triangle_mode);
    toolbar->addAction(triangle_action);

    // 添加保存当前视图为图片的按钮
    QAction *save_action = new QAction(QIcon(":/IMG/icons/camera.png"), "保存当前视图为图�?", this);
...
// {
//     QString folder = QFileDialog::getExistingDirectory(this, "选择保存文件�?");
//     if (!folder.isEmpty())
//     {
...
//             showTooltip(QString("已移�? '%1'").arg(currentFilename));
//         }
//         else
//         {
//             lines.append(currentFilename);
//             showTooltip(QString("已添�? '%1'").arg(currentFilename));
//         }
...
//         QStringList lines;
//         lines.append(currentFilename);
//         showTooltip(QString("文件不存在。新�? '%1'，并添加 '%1'").arg(currentFilename));
//     }
